...
게이트웨이가 메모리를 쓰는 곳은 요청 본문, upstream 응답, 캐시 세 가지다. 셋 다 상한을 둔다.

| 설정 | 기본값 | 막는 것 |
|---|---|---|
| `server.max_inflight` | 512 | 동시에 붙잡는 요청 수. 넘치면 본문을 읽기 전에 503(`Retry-After: 1`). 대기열에서 기다리는 요청도 본문을 들고 있으므로 이 값이 요청 메모리의 상한이다 |
| `server.max_request_bytes` | `"32MiB"` | chat 요청 본문 하나. 넘으면 413 |
| `provider.max_response_bytes` | `"8MiB"` | 일반 응답 본문, 스트림 한 줄. 넘으면 그 시도는 실패하고 다음 step 으로(줄바꿈 없이 큰 데이터가 와도 버퍼가 커지지 않는다) |
| `cache_max_bytes` | `"64MiB"` | 응답 캐시 전체. 항목 수 상한 1024 와 함께 적용하고, 넘치면 오래된 항목부터 버린다 |
| `server.memory_limit` | 없음 | Go 런타임 메모리 목표(`GOMEMLIMIT` 과 같다). 이 근처에 오면 GC 를 더 돌린다. 컨테이너 한도의 80~90% 로 둔다 |

에러 응답 본문은 앞 4KiB 만 읽는다. passthrough 는 본문을 메모리에 모으지 않고 흘려보낸다.
최악의 요청 메모리는 대략 `max_inflight × max_request_bytes × 3`(원문·마스킹 사본·직렬화) 이다.
실제 요청이 작으면 훨씬 적게 쓰지만, 컨테이너 한도를 정할 때 이 값을 기준으로 삼는다.
`/stats` 의 `memory` 에서 힙 크기, 캐시 바이트, 지금 붙잡은 요청 수, 503 으로 돌려보낸 수를 볼 수 있다.
...
- 응답 캐시는 최대 1,024건, `cache_max_bytes`(기본 64MiB) 이하다. 크기는 본문·추론·시도 기록으로 어림한 값이다.
